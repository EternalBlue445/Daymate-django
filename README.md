...
# How to use it? 
Render is a free hosting platform, so upon inactivity it suspends the server(frontend and backend). So, to use it go to the following urls. Backend first and then frontend. Wait for 1 minute. Render will then restart the servers. When backend and frontend both are running then change location to see new recommendation.
